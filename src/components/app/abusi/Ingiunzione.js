...
class Ingiunzione extends React.Component{
  ...
  componentDidMount(){
    ...
    $.ajax({
        type: 'GET',
        //data: formData,
        url: constants.DB_ADDR+'getIngiunzione?pid='+this.props.pid+'&dbid='+this.props.dbid,
        processData: false,
        contentType: false,
  ...
  downloadModulo(){
    window.open(LINKS.ingiunzionetipo, '_blank')
  }
      ...
      if( this.state.data.length == 0 ){
        tableContents.push(
          ...
          for ( var i = 0; i < this.state.data.length; i++){
            tableContents.push(
              <TableRow key={i}>
                <TableRowColumn>Ingiunzione {i+1}</TableRowColumn>
      ...
      return (
          <Box column style={{marginTop:'30px', width:'98%'}} alignItems="flex-start" justifyContent="flex-start">
              <Toolbar style={{backgroundColor:'#4CA7D0', width:'100%'}}>
                <ToolbarTitle text="File caricato per Ingiunzione" style={{color:'#FFFFFF', textAlign:'center', fontSize:'15px'}}/>
