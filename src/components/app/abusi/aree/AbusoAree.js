...
class GestioneAbusi extends React.Component{
  ...
  componentDidMount(){
    ...
    $.ajax({
        type: 'GET',
        //data: formData,
        url: constants.DB_ADDR+'getAbusiAree?cid='+escape(global.city)+"&offset="+this.state.offset,
        processData: false,
        contentType: false,
  ...
  onSearchChange(e,v){
    ...
    if(v === ''){
      ...
      $.ajax({
          type: 'GET',
          url: constants.DB_ADDR+'searchTableD?search='+escape(v)+'&cid='+escape(global.city),
          processData: false,
          contentType: false,
          success: function(data) {
            var parsed = JSON.parse(data);
