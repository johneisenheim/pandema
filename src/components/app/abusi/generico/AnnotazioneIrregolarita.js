...
class AnnotazioneIrregolarita extends React.Component{
  ...
  componentDidMount(){
    ...
    $.ajax({
        type: 'GET',
        //data: formData,
        url: constants.DB_ADDR+'annotazioneIrregolaritaAbusi?pid='+this.props.pid+'&dbid='+this.props.dbid,
        processData: false,
        contentType: false,
      ...
      if( this.state.data.length == 0 ){
        tableContents.push(
          ...
          for ( var i = 0; i < this.state.data.length; i++){
            tableContents.push(
              <TableRow key={i}>
                <TableRowColumn>File Annotazione Irregolarità #{i+1}</TableRowColumn>
      ...
      return (
          <Box column style={{marginTop:'30px', width:'97%'}} alignItems="flex-start" justifyContent="flex-start">
              <Toolbar style={{backgroundColor:'#4CA7D0', width:'100%'}}>
                <ToolbarTitle text="Files caricati per Annotazione Irregolarità" style={{color:'#FFFFFF', textAlign:'center', fontSize:'15px'}} onTouchTap={this.downloadModulo.bind(this)}/>
                <ToolbarGroup style={{marginRight:'0px'}}>
                  <FlatButton label="Allega File" icon={<Attach style={{fill:'#FFFFFF'}}/>} style={{marginTop:'10px', marginRight:'0px'}} labelStyle={{color:'#FFFFFF'}}>
                    <input type="file" accept="application/pdf" style={styles.inputFile} onChange={this._domandeConcorrenzaFileHandler.bind(this)} ref="file"/>
                  </FlatButton>
                </ToolbarGroup>
              </Toolbar>
...
export default AnnotazioneIrregolarita;
