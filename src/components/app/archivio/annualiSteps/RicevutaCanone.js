...
class RicevutaCanone extends React.Component{
  ...
  componentDidMount(){
    ...
    $.ajax({
        type: 'GET',
        //data: formData,
        url: constants.DB_ADDR+'getRicevuteCanone?pid='+this.props.pid+'&dbid='+this.props.dbid,
        processData: false,
        contentType: false,
      ...
      if( this.state.data.length == 0 ){
        tableContents.push(
          ...
          for ( var i = 0; i < this.state.data.length; i++){
            tableContents.push(
              <TableRow key={i}>
                <TableRowColumn>Ricevuta Canone {i+1}</TableRowColumn>
