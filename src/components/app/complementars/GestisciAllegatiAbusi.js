...
class GestisciAllegatiAbusi extends React.Component{
  ...
  componentDidMount(){
    ...
    $.ajax({
        type: 'GET',
        url: constants.DB_ADDR+'getAllegatiAbusi?praticaID='+_self.props.params.dbid+'&pandemaPraticaID='+_self.props.params.pid,
        processData: false,
        contentType: false,
        success: function(data) {
          console.log('success');
          var parsed = JSON.parse(data);
          console.log('GestisciAllegatiAbusi()',parsed);
          _self.setState({
            ..._self.state,
            isLoading : false,
  ...
  eyePress(pratica_id, allegato_id, pratica_pandema_id){
    ...
    $.ajax({
        type: 'GET',
        url: constants.DB_ADDR+'viewDocumentAbuso?allegatoID='+escape(allegato_id),
        processData: false,
        contentType: false,
        success: function(data) {
  ...
  deletePress(path, allegato_id){
    window.open(constants.DB_ADDR+'downloadFileAbuso?id='+allegato_id, '_blank')
  }
  ...
  createTable(){
    ...
    for( var i = 0; i < this.state.results.length; i++ ){
      tableContents.push(
        <TableRow key={i}>
          <TableRowColumn>{this.state.results[i].descrizione_com}</TableRowColumn>
          <TableRowColumn>{new Date(this.state.results[i].data_creazione).toLocaleDateString()}</TableRowColumn>
          <TableRowColumn>
            <IconButton onTouchTap={this.eyePress.bind(this, this.state.results[i].pratica_id, this.state.results[i].id, this.state.results[i].pratica_pandema_id)}><Eye color="#909EA2"/></IconButton>
            <IconButton onTouchTap={this.deletePress.bind(this, this.state.results[i].path, this.state.results[i].id)}><Download color="#909EA2"/></IconButton>
          </TableRowColumn>
        </TableRow>
      );
    }
...
export default GestisciAllegatiAbusi;
