...
class DomandeConcorrenza extends React.Component{
  ...
  componentDidMount(){
    ...
    $.ajax({
        type: 'GET',
        //data: formData,
        url: constants.DB_ADDR+'d1domandeconcorrenza?pid='+this.props.pid+'&dbid='+this.props.dbid,
        processData: false,
        contentType: false,
              ...
              <Toolbar style={{backgroundColor:'#4CA7D0', width:'100%'}}>
                ...
                <ToolbarGroup style={{marginRight:'0px'}}>
                  <FlatButton label="Allega File" icon={<Attach style={{fill:'#FFFFFF'}}/>} style={{marginTop:'10px', marginRight:'0px'}} labelStyle={{color:'#FFFFFF'}}>
                    <input type="file" style={styles.inputFile} accept="application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" onChange={this._domandeConcorrenzaFileHandler.bind(this)} ref="file"/>
                  </FlatButton>
                </ToolbarGroup>
              </Toolbar>
