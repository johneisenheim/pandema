...
class RichiestaAdempimenti extends React.Component{
  ...
  componentDidMount(){
    ...
    $.ajax({
        type: 'GET',
        //data: formData,
        url: constants.DB_ADDR+'getRichiestaAdempimenti?pid='+this.props.pid+'&dbid='+this.props.dbid,
        processData: false,
        contentType: false,
  ...
  downloadModulo(){
    window.open(LINKS.adempimentid3, '_blank')
  }
      ...
      if( this.state.data.length == 0 ){
        tableContents.push(
          ...
          for ( var i = 0; i < this.state.data.length; i++){
            tableContents.push(
              <TableRow key={i}>
                <TableRowColumn>File Adempimenti {i+1}</TableRowColumn>
      ...
      return (
          <Box column style={{marginTop:'30px', width:'100%'}} alignItems="flex-start" justifyContent="flex-start">
              <Toolbar style={{backgroundColor:'#4CA7D0', width:'100%'}}>
                <ToolbarTitle text="Files caricati per Adempimenti" style={{color:'#FFFFFF', textAlign:'center', fontSize:'15px'}}/>
                <ToolbarGroup style={{marginRight:'0px'}}>
                  ...
                  <FlatButton label="Allega File" icon={<Attach style={{fill:'#FFFFFF'}}/>} style={{marginTop:'10px', marginRight:'0px'}} labelStyle={{color:'#FFFFFF'}}>
                    <input type="file" accept="application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" style={styles.inputFile} onChange={this._avvisoPubblicazioneFileHandler.bind(this)} ref="file"/>
                  </FlatButton>
                </ToolbarGroup>
              </Toolbar>
...
export default RichiestaAdempimenti;
