...
class AvvisoPubblicazione extends React.Component{
  ...
  componentDidMount(){
    ...
    $.ajax({
        type: 'GET',
        //data: formData,
        url: constants.DB_ADDR+'d1avvisopubblicazione?pid='+this.props.pid+'&dbid='+this.props.dbid,
        processData: false,
        contentType: false,
  ...
  downloadModulo(){
    window.open(constants.DB_ADDR+'downloadAvvisoPubblicazione', '_blank')
  }
      ...
      if( this.state.data.length == 0 ){
        tableContents.push(
          ...
          for ( var i = 0; i < this.state.data.length; i++){
            tableContents.push(
              <TableRow key={i}>
                <TableRowColumn>Avviso Pubblicazione {i+1}</TableRowColumn>
